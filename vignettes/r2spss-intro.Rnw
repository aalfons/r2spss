...
tests, ANOVA, and linear regression, as well as boxplots, histograms,
scatterplots, and line plots (including profile plots).
}
...
correctly.  For portability reasons, this vignette therefore only displays
\proglang{LaTeX} tables that mimic the simpler look of older \proglang{SPSS}
versions.  For convenience, such a global preference within an \proglang{R}
session can be set with the accessor function \code{r2spssOptions$set()}.
