...
ttest <- function(data, variables, group = NULL, mu = 0, conf.level = 0.95) {
  ...
  if (length(group) == 0) {
    if (length(variables) == 1) {
      ...
      ok <- is.finite(x)
      x <- x[ok]
      t <- t.test(x, mu=mu, conf.level=conf.level)
      # construct object
      out <- list(statistics=stat, test=t, variables=variables[1],
                  type="one-sample")
    } else {
      ...
      x <- x[ok]
      y <- y[ok]
      t <- t.test(x, y, mu=0, paired=TRUE, conf.level=conf.level)
      # construct object
      out <- list(statistics=stat, test=t, variables=variables[1:2],
